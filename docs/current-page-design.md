...
- 三个标签：待处理、正式课、已知悉充值。
- 待处理把课程与充值待办合并展示，支持修改、删除与明细折叠；单次班课直接显示教练上报人数，不显示会员消费明细；允许部分接口失败时保留已加载数据。
- 正式课提供当月课程数、总时长、等效总人数，并支持近三个月分页。单次班课按一节课程计数，上报人数按 1:1 计入等效总人数。
...
- 课程类型包括体验课未成单、体验课成单、订场、班课、私教、单次班课。
- 订场、班课、私教可搜索会员并填写扣费方式、金额/次数、折算金额、数量；检测到欠费时提示但不阻止提交。单次班课不选择会员，必须填写正整数上报人数。
...
历史截图不代表本次团课页面。底部五个 Tab 仅文字和选中颜色；团课列表、详情、创建页共用 manager_permissions 全局权限。授课下拉来自 CloudBase manager.name，资料引用 manager._id，不读取独立 coach 集合。模板仅填充标题详情；没有草稿，发布前弹框确认，发布与占场一次提交，发布后不可修改。所有运行时团课 mock 已移除。

完整现行设计见 docs/superpowers/specs/2026-08-24-group-course-design.md（Obsidian 同步副本）。手动部署清单在 Obsidian court-book/团课部署与验收。

创建表单手填原价与必填VIP实付金额（正整数元，VIP不高于原价），确认弹框展示填写值。未管理校区无court记录时由服务端提供1号场、2号场；下拉显示完整编号标签，切换校区清空选择。有真实场地时使用真实列表。
